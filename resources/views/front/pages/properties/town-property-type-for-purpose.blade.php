            ...
            <div class="row">
                <div class="col-md-8 offset-md-2 text-center">
                    <div class="breadcrumb-menu">
                        ...
                        <span>
                            <a
                                href="{{ route('cpt-purpose', [$buyOrRent,$city->slug,Str::slug($type->plural) . '-for-' . strtolower($property_purpose)]) }}">
                                {{ $city->name }}</a>
                        </span>
                        <span>
                            <a
                                href="{{ route('cpt-purpose', [$buyOrRent,$city->slug,Str::slug($type->plural) . '-for-' . strtolower($property_purpose) . '-' . $subcity->slug]) }}">
                                {{ $subcity->name }}
                            </a>
                        </span>
                        <span>{{ $town->name }}</span>
                        ...
                        <div class="row d-flex property-filter d-sm-flex col-12">
                            ...
                            @foreach ($areas as $item)
                                <div class="col-3 {{ $loop->index > 7 ? 'moreLess' : '' }}">
                                    <a href="{{ route('cpt-purpose', [$buyOrRent,Str::slug($city->slug),Str::slug($type->plural) .'-for-' .strtolower($property_purpose) .'-' .$subcity->slug .'-' .$town->slug .'-' .$item->slug]) }}"
                            ...
                            @if (count($properties) > 0)
                                ...
                                <div class="property-filter d-sm-flex mt-10">
                                    <ul class="property-short list-unstyled d-sm-flex mb-0">
                                        <li>
                                            <form
                                                action="{{ route('cpt-purpose', [$buyOrRent,Str::slug($city->slug),Str::slug($type->plural) . '-for-' . strtolower($property_purpose) . '-' . $subcity->slug . '-' . $town->slug]) }}"
