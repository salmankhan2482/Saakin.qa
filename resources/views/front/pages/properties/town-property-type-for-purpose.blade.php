      ...
      @else
         <div class="mb-3">
            <h1 class="h6">{{ $heading_info ?? '' }}
               <small class="d-block fs-sm fw-normal mt-2">
                  {{ count($properties) > 0 ? $properties->total() . 'results' : '' }}
               </small>
            </h1>
         </div>
                  ...
                  <div class="single-property-box horizontal-view" @if (!(new \Jenssegers\Agent\Agent())->isMobile())  @endif>
                     <div class="property-item">
                           <div class="pro-slider">
                              <div class="pro-slider-item">
                                 @if (!(new \Jenssegers\Agent\Agent())->isDesktop())
                                    <img src="{{ asset('upload/m_properties/mobile_thumb_' . $property->featured_image) }}" alt="{{ $property->property_name }}">
                                 @else
                              ...
                              @if (count($property->gallery) > 0)
                                 @foreach ($property->gallery as $gallery)
                                    @if ($loop->index < 5)
                                       <div class="pro-slider-item">
                                          @if (!(new \Jenssegers\Agent\Agent())->isDesktop())
                                             <img src="{{ asset('upload/m_gallery/') . '/mobile_' . $gallery->image_name }}" alt="{{ $property->property_name }}">
                                          @else
                     ...
                     @if ((new \Jenssegers\Agent\Agent())->isTablet() || 
                        (new \Jenssegers\Agent\Agent())->isDesktop())
                           <div class="property-card-extra p-3 d-none d-md-block">
                              <div class="property-type">
                                 @if ($property->featured_property == 1)
                                       Featured
