                            ...
                            @foreach ($propertyTypes as $propertyType)
                                <div class="col-3 {{ $loop->index > 7 ? 'moreLess' : ''}}">
                                    @if (ucfirst($property_purpose) == 'Sale')
                                        <a 
    href="{{ route('property-type-purpose',['buy', Str::slug($propertyType->plural) . '-for-'.$property_purpose]) }}"
                                        >
                                        <span style="color: #009FFF">
                                        @foreach (explode('-', $propertyType->plural) as $key )
                                    ...
                                    @else
                                        <a 
    href="{{ route('property-type-purpose',['rent', Str::slug($propertyType->plural) . '-for-'.$property_purpose]) }}"
                                        >
                                            <span style="color: #009FFF">
                                                @foreach (explode('-', $propertyType->plural) as $key )
                                                    <span>{{ ucfirst( $key ) }}</span>
                                            ...
                                            <select name="property_type" id="property_type"
                                                ...
                                                @foreach ($propertyTypes as $propertyType)
                                                    <option value="{{ $propertyType->id }}">
                                                        {{ $propertyType->plural }} ({{ $propertyType->pcount }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <div class="property-filter d-sm-flex mt-10">
                                    <ul class="property-short list-unstyled d-sm-flex mb-0">
                                        <li>
                                            ...
                                            <form 
                                                action="{{ route('property-purpose', [$buyOrRent, $property_purpose]) }}" name="frmSortBy" 
                                                id="frmSortBy"
