        ...
        <div class="form-head page-titles d-flex  align-items-center">
            <div class="mr-auto  d-lg-block">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="javascript:void(0)">{{ $property->address }}</a></li>
                </ol>
            </div>
                ...
                <div class="row">
                    <div class="col-xl-12">
                        <div class="card bg-primary text-center">
                            <div class="card-body">
                                <h2 class="fs-30 text-white">{{ $property->property_purpose }}</h2>
                                <span class="text-white font-w300">{{ $property->getPrice() }}</span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-body">
                                <div class="position-relative mb-3 d-inline-block">
                                    <img src="{{ asset('upload/properties/' . $property->agent_picture) }}"
                                        alt="{{ $property->agent_name }}" width="140">
                                    {{-- <a href="app-profile.html" class="profile-icon"><i class="las la-cog"></i></a> --}}
                                </div>

                                <h4 class="text-black fs-20 font-w600">{{ $property->agent_name ?? 'This has no Agent' }}
                                </h4>
                                <span class="mb-3 text-black d-block">Agent</span>
                                <small>
                                    <h6 class="text-black fs-14 font-w600">{{ $agency->name ?? '' }}</h6>
                                </small>
                                <span class="mb-3 text-black d-block">Agency</span>

                                <p>{{ $agency->head_office ?? 'No Address Available' }}</p>
                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-body">
                                <div class="front-view-slider mb-sm-5 mb-3 owl-carousel">
                                    <div class="items">
                                        <div class="front-view">
                                            ...
                                            {{-- <div class="info"> --}}
                                            {{-- // Title Heading --}}
                                            {{-- <h3 class="fs-30 mb-2 text-white font-w600"> </h3> --}}
                                ...
                                <div>
                                    <div class="btn btn-primary rounded mb-4">{{ $property->refference_code }}</div>
                                    <div class="d-md-flex d-block mb-sm-5 mb-3">
                                        <div class="mr-auto mb-md-0 mb-4">
                                            <h2 class="font-w600 text-black">{{ $property->property_name }}</h2>
                                            <span class="fs-18">
                                        ...
                                        <div class="ml-md-4 text-md-right">
                                            ...
                                            <h4 class="fs-24 text-primary">{{ $property->getPrice() }}</h4>
                                        </div>
                                    </div>
                                    <div class="mb-sm-5 mb-2">
                                        <a href="javascript:void(0);"
                                            class="btn btn-primary light rounded mr-2 mb-sm-0 mb-2">
                                            <i class="las la-solid la-building"></i>
                                            ...
                                            {{ $property->propertiesTypes->types ?? 'No' }}
                                        </a>
                                        <a href="javascript:void(0);"
                                            class="btn btn-primary light rounded mr-2 mb-sm-0 mb-2">
                                            <i class="las la-solid la-bed"></i>
                                            {{-- <svg class="mr-2" width="28" height="19" viewBox="0 0 28 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M23.1 8.03846C25.7498 8.03846 28 10.2859 28 13.1538V17.5385H25.9V19H24.5V17.5385H3.5V19H2.1V17.5385H0V13.1538C0 10.3876 2.17398 8.03846 4.9 8.03846H23.1ZM21.7 0C23.5821 0 25.2005 1.57962 25.2 3.65385L25.2005 7.14522C24.5639 6.78083 23.8517 6.57692 23.1 6.57692L21.7 6.57618C21.7 5.32466 20.7184 4.38462 19.6 4.38462H15.4C14.8622 4.38462 14.3716 4.59567 14.0001 4.94278C13.629 4.59593 13.1381 4.38462 12.6 4.38462H8.4C7.24044 4.38462 6.30038 5.36575 6.3 6.57619L4.9 6.57692C4.14851 6.57692 3.43653 6.7807 2.8 7.14488V3.65385C2.8 1.68899 4.3096 0 6.3 0H21.7ZM12.6 5.84579C12.9799 5.84579 13.3 6.21117 13.3 6.57692L7.7 6.57618C7.7 6.12909 8.04101 5.84615 8.4 5.84615L12.6 5.84579ZM19.6 5.85107C19.9961 5.84578 20.2996 6.20175 20.3 6.57618H14.7C14.7 6.1227 15.041 5.84615 15.4 5.84615L19.6 5.85107Z" fill="#3B4CB8"/>
                                        </svg> --}}
                                            {{ $property->bedrooms ?? 'No' }} Bedroom
                                        </a>
                                        <a href="javascript:void(0);"
                                            class="btn btn-primary light rounded mr-2 mb-sm-0 mb-2">
                                            <i class="las la-solid la-bath"></i>
                                ...
                                <ul>
                                    @if ($property->property_features)
                                        @foreach (explode(',', $property->property_features) as $features)
                                            ...
                                            @endphp
                                            @if ($ameniti)
                                                <li><i class="las la-check-circle"></i>{{ $ameniti }}</li>
                                            @endif
                                        @endforeach
                                    @else
                                        @foreach ($property->amenities as $amenity)
                                            <div class="fw-bold">
                                                <li><i class="las la-check-circle"></i> {{ $amenity->name }}</li>
                                            </div>
                                        @endforeach
                                    @endif
...
@endsection
@section('style')
    <link src="{{ asset('admin/vendor/jqvmap/css/jqvmap.min.css') }}" rel="stylesheet" type="text/css" />
@endsection
@section('scripts')
