...
class BlogController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if($blog_image) {
            ...
            $blog_image_name = explode(".",$blog_image_name);
            $tmpFilePath = public_path('upload/blogs/');
            $imageName = $blog_image_name[0].'_'.time().'.'.$blog_image->extension();
            $blog_image->move($tmpFilePath, $imageName);
            $blog_image_new_name = $imageName;
            \File::delete(public_path() .'/upload/blogs/'.$blog->image);
            $blog->image = $blog_image_new_name;
        }
