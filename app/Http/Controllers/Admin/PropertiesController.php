...
class PropertiesController extends MainAdminController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($featured_image) {
            ...
            $featured_image_name = implode(".", $featured_image_name);
        ...
        if ($property_gallery_files) {
            foreach ($property_gallery_files as $file) {
                ...
                $gallery_image_name = implode(".", $gallery_image_name);
                $img = Image::make($file->getRealPath());
                $img->resize(861, 608, function ($constraint) {
                    $constraint->aspectRatio();
                })->save($gallery_image_path.'/'.$name);
                $img->resize('124', '182')->save(public_path() . '/upload/m_gallery/' . 'mobile_property_'.$name);
    ...
    public function edit(Request $request, $id)
    {
        ...
        // dd($data['property_gallery_images']);
        $data['cities'] = PropertyCities::all();
        $data['subCities'] = PropertySubCities::all();
        $data['towns'] = PropertyTowns::all();
        $data['areas'] = PropertyAreas::all();
    ...
    public function update(Request $request, $id)
   {
      ...
      if ($featured_image) {
          ...
          $resize = $tmpFilePath . 'thumb_' . $name;
          $image_resize->save($resize);
          $image_resize->resize('124', '182')->save(public_path() . '/upload/featured/' . 'mobile_thumb_'.$featured_image_name);
      ...
      if ($featured_image) {
         // deleting thumb and featured image from properties folder for desktop
         dump(File::delete(public_path() . '/upload/properties/' . $property->featured_image));
         dump(File::delete(public_path() . '/upload/properties/thumb_' . $property->featured_image));
         
         // deleting thumb or featured image from properties folder for mbl
         dump(File::delete(public_path() . '/upload/m_properties/mobile_thumb_' . $property->featured_image));
      ...
      if ($agent_picture) {
         ...
         // deleting agent pic for desktop
         dump(File::delete(public_path() . '/upload/properties/' . $property->agent_picture));
         
         // deleting agent pic for mbl
         dump(File::delete(public_path() . '/upload/m_properties/' . $property->agent_picture));
