...
<head>
    ...
    <img href="<?php echo e(URL::asset('upload/'.getcong('site_favicon'))); ?>" rel="shortcut icon" type="image/x-icon" />
    ...
    <script type="text/javascript" src="<?php echo e(asset('site_assets/ckfinder/ckfinder.js')); ?>"></script>

    <?php
    $settings = App\Settings::where("id",1)->get()->first();
    ?>
    
    <script src="https://maps.googleapis.com/maps/api/js?libraries=places&amp;key=<?php echo e($settings->google_map_key); ?>&callback=initMap&region=qa" type="text/javascript"></script>
    <style>
        li.parsley-required{
...
<!-- Plugins -->
<script src="<?php echo e(URL::asset('assets/js/jquery.validate.js')); ?>"></script>
...
<script>
    ...
    $(document).on('click',"#savegomaster",function (e) {
        ...
        if($("#importPropertiesform").valid()){
            $.ajax({
                type: "POST",
                url: "<?php echo e(url('api/get-agencies')); ?>",
                dataType : 'JSON',
                beforeSend: function () {
    ...
    function ImportAgencies(xml_record_id,chunk_id){
        ...
        new Promise(function(resolve, reject) {
            $.ajax({
                type: "POST",
                url: "<?php echo e(url('api/import-agencies/chunks')); ?>",
                dataType: 'JSON',
                data: {
...
<?php /**PATH C:\xampp 7.4\htdocs\saakin\resources\views/admin/admin_app.blade.php ENDPATH**/ ?>